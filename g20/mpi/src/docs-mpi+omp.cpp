...
int associate_cabin(int init, int end, int rank, int size) {
    ...
    int *ptr_local_cabins_size = local_cabins_size.data();

    #pragma omp parallel for reduction(+ : ptr_local_cabins_size[0 : Cabinets])
    for (int i = init; i < end; i++) {
        associated_cabins[i] = i % Cabinets;
        ptr_local_cabins_size[i % Cabinets] += 1;
    }
    ...
    while (changed) {
        ...
        double *ptr_local_cabins_values = local_cabins_values.data();
        int *ptr_local_deltas = local_cabins_size_deltas.data();

        #pragma omp parallel 
        {
            #pragma omp for reduction(+ : ptr_local_cabins_values[0 : Cabinets * Subjects])
            for (int id = init; id < end; id++) {
                int cabin = associated_cabins[id];
                for (int subject = 0; subject < Subjects; subject++) {
                    ptr_local_cabins_values[(cabin * Subjects) + subject] +=
                        subject_scores[(id * Subjects) + subject];
                }
            }

            #pragma omp single
            {
                MPI_Allreduce(local_cabins_values.data(), cabins_values.data(),
                              Cabinets * Subjects, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            }
            ...
            #pragma omp for reduction(+ : ptr_local_deltas[0 : Cabinets]) reduction(|| : local_changed)
            for (int id = init; id < end; id++) {
                ...
                if (new_associated != current_associated) {
                    ptr_local_deltas[new_associated]++;
                    ptr_local_deltas[current_associated]--;
            ...
            #pragma omp single
            {
                ...
                MPI_Allreduce(local_cabins_size_deltas.data(), global_deltas.data(), Cabinets,

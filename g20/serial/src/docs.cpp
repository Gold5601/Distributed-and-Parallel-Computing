...
int associate_cabin() {
    ...
    while (changed) {
        ...
        int id = 0;
        for (int cabin: associated_cabins ) {
            // for each subject inside the document scores
            for (int subject = 0; subject < Subjects; subject++) {
                cabins_values[(cabin * Subjects) + subject] += subject_scores[(id * Subjects) + subject];
            }
            id++;
        }
        ...
        for (int id = 0; id < Documents; id++) {
            ...
            if (new_associated != current_associated) {
                cabins_size[new_associated]++;
                cabins_size[current_associated]--;
